...
class TrackInfo(Base, TypedInfo):
    ...
    def as_dict(self, genre=False, album=False):
        return {
            ...
            "genre": self.genre.as_dict()
            if (genre and "genre" in self.__dict__)
            else None,
            "album": self.album.as_dict(artist=False)
            if (album and "album" in self.__dict__)
            else None,
...
class Artist(Base, TypedInfo):
    ...
    def as_dict(self, albums=False, tracks=False):
        return {
            "id": self.id,
            "name": self.name,
            "albums": [x.as_dict(tracks=tracks) for x in self.albums]
            if (albums and "albums" in self.__dict__)
            else None,
        }
...
class Album(Base, TypedInfo):
    ...
    def as_dict(self, artist=False, tracks=False):
        return {
            ...
            if (artist and "artist" in self.__dict__)
            else None,
            "name": self.name,
            "year": self.year,
            "tracks": [x.as_dict() for x in self.tracks]
            if (tracks and "tracks" in self.__dict__)
            else None,
        }
...
class Context:
    ...
    def __enter__(self):
        ...
        if self.load:
            ...
            for mode in self.config["SCANNER_MODES"]:
                self.infos[mode] = {}
                self.infos[mode]["artists"] = {
                    x.name.strip().lower(): x
                    for x in self.session.query(Artist)
                    .join(Artist.albums)
                    .options(contains_eager(Artist.albums))
                    .filter(Artist.type == mode)
                    .all()
                }
                self.infos[mode]["genres"] = {
                    x.name.strip().lower(): x
                    for x in self.session.query(Genre).filter(Artist.type == mode).all()
                }
        return self
    ...
    def delete_orphans(self):
        self.session.query(Genre).filter(
            ~Genre.id.in_(self.session.query(distinct(TrackInfo.genre_id)))
        ).delete(False)
        self.session.commit()
        self.session.query(Album).filter(
            ~Album.id.in_(self.session.query(distinct(TrackInfo.album_id)))
        ).delete(False)
        self.session.commit()
        self.session.query(Cover).filter(
            ~Cover.id.in_(self.session.query(distinct(Album.cover_id)))
        ).delete(False)
        self.session.commit()
        self.session.query(Artist).filter(
            ~Artist.id.in_(self.session.query(distinct(Album.artist_id)))
        ).delete(False)
        self.session.commit()
    ...
    def delete_tracks(self, tracks_path):
        tracks = (
            ...
            .join(Track.infos)
            .filter(Track.path.in_(tracks_path))
            .options(joinedload(Track.infos))
            .all()
        )
        for track in tracks:
    ...
    def get_album_path(self, album):
        res = (
            self.session.query(Track.path)
            .join(TrackInfo)
            .filter(TrackInfo.album_id == album.id)
            .first()
        )
    ...
    def add_track_info(
        ...
        if (
            trackinfo.album.last_updated is None
            or trackinfo.album.last_updated < track.last_updated
        ):
