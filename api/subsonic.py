...
def get_filter_by_type(atype, fromYear=None, toYear=None, genre=None):
    if atype == "newest":
        return lambda query: query.add_columns(
            func.max(Album.last_updated.label("last_updated"))
        ).group_by(Album.id)
    elif atype == "starred":
...
def format_album(album, child=False):
    ...
    info = {
        ...
        # TODO "created": min(map(lambda t: t.last_updated, album.tracks)).isoformat(),
        "year": album.year,
                ...
                "index": [
                    {
                        "name": k,
                        "artist": [
                            {"id": format_artist_id(a.id), "name": a.name} for a in v
                        ],
                    }
    ...
    return request.formatter(
        {
            "album": {
                ...
                "songCount": al[0].track_count(),
                "duration": int(al[0].duration()),
                "created": datetime.now().replace(microsecond=0).isoformat(),
            }
        }
    )
...
def _album_list(typed):
    ...
    if not ok:
        return False, size
    ok, offset = check_parameter(
        request, "offset", fct=lambda val: int(val) if val else 0
    )
    if not ok:
        return False, offset
    ...
    if atype == "byYear" and (not fromYear or not toYear):
        return (
            False,
            request.error_formatter(10, "Missing parameter fromYear or toYear"),
        )
...
def random_songs(typed):
    ...
    def gen():
        if count > 0:
            for _ in range(size):
                x = random.choice(range(count))
                yield typed.gettrackinfo(
                    ffilter=lambda query: query.filter(*fltr).offset(x).limit(1)
                )

    return request.formatter(
        {"randomSongs": {"song": [format_track(track, child=True) for track in gen()]}}
    )
...
def album_list(typed):
    ...
    return request.formatter(
        {"albumList": {"album": [format_album(album, child=True) for album in albums]}}
    )
...
def album_list2(typed):
    ...
    return request.formatter(
        {"albumList2": {"album": [format_album(album) for album in albums]}}
    )
...
def search2and3(typed):
    ...
    ok, artistCount = check_parameter(
        request, "artistCount", fct=lambda val: int(val) if val else 20
    )
    if not ok:
        return False, artistCount
    ok, artistOffset = check_parameter(
        request, "artistOffset", fct=lambda val: int(val) if val else 0
    )
    if not ok:
        return False, artistOffset
    ok, albumCount = check_parameter(
        request, "albumCount", fct=lambda val: int(val) if val else 20
    )
    if not ok:
        return False, albumCount
    ok, albumOffset = check_parameter(
        request, "albumOffset", fct=lambda val: int(val) if val else 0
    )
    if not ok:
        return False, albumOffset
    ok, songCount = check_parameter(
        request, "songCount", fct=lambda val: int(val) if val else 20
    )
    if not ok:
        return False, songCount
    ok, songOffset = check_parameter(
        request, "songOffset", fct=lambda val: int(val) if val else 0
    )
    if not ok:
...
def cover_art(typed):
    ...
    if cover is None or cover.mbid == "0":
        ...
        return send_from_directory(
            Thumb.getdir(), os.path.basename(cover.path), conditional=True
        )
